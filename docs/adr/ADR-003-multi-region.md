...
- **Status:** Proposed (detailed design; Y2Q2 OKRs)  
- **Date:** 2026-08-07  
...
1. **Now:** Single primary region production; PII map and residency *plan* exist — do not fake multi-region.  
2. **Y2Q2:** Deploy Atlas **multi-region** (or equivalent) with clear primary write region and secondary read/failover strategy.  
3. **API:** Introduce regional routing only after data layer supports it; measure RTT budgets for poll/long-poll.  
4. **Realtime bus:** Prefer co-locating event storage with the API region; if cross-region poll RTT breaks SLOs, trigger ADR-001 addendum (dedicated edge).  
5. **Media:** WebRTC mesh only on this deploy ([ADR-002](./ADR-002-sfu-evaluation.md)); TURN/ICE via `/api/ice`.  
6. **DR:** Quarterly failover game days; RPO/RTO published before enterprise contracts that require them.
